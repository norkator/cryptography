...
public class AdaptiveHuffmanDecode {
			...
			if(tree.get(currNode).left==-1&&tree.get(currNode).right==-1){
				if(currNode==NYT){
					...
					if(p<10){
						code+=data.charAt(a++);
						p=Integer.parseInt(code,2);
						output+=intTocode(p,5);
					} else {
						output+=intTocode(p,4);
					}
				} else {
					output+=tree.get(currNode).ch;
				}
	...
	public static String intTocode(int x,int bit) {
		if(bit==5){
			if(x==0){
				return "a";
			} else if(x==1){
	...
	static boolean first(String str){
	...
	static void display() {
		for(int a=0;a<tree.size();a++) {
			System.out.println(""+a+" ch:"+tree.get(a).ch+" weight:"+tree.get(a).weight+" no:"+tree.get(a).no+" l:"+tree.get(a).left+" r:"+tree.get(a).right+" p:"+tree.get(a).parent+" code:"+tree.get(a).code);
		}
